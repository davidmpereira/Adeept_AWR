    ...
    while True:
        try:
            ...
            try:
                ...
                cv2.putText(source,('LAT: %s'%GPSLAT),(370,440), font, 0.5,(128,255,128),1,cv2.LINE_AA)
                cv2.putText(source,('LON: %s'%GPSLON),(500,440), font, 0.5,(128,255,128),1,cv2.LINE_AA)
                cv2.putText(source,('GYRO: %s'%GYRO),(370,470), font, 0.5,(128,255,128),1,cv2.LINE_AA)
...
def call_SportMode(event):
    global SportModeOn
    if SportModeOn:
        tcpClicSock.send(('SportModeOff').encode())
    else:
...
def Info_receive():
    global CPU_TEP,CPU_USE,RAM_USE, GPS, GPSLAT, GPSLON, GYRO
    HOST = ''
    ...
    while 1:
        try:
            ...
            CPU_TEP,CPU_USE,RAM_USE,GPS,GYRO = info_get
            if "," in GPS:
                GPSLAT = GPS.split(",")[0]
                GPSLON = GPS.split(",")[1]
            else:
